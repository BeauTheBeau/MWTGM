    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand => subcommand
            ...
            .addStringOption(option => option
                ...
                .setRequired(true)
                .addChoices(
                    {name: `Coding Culture`, value: 'Coding Culture'},
                    {name: `Poutine`, value: 'poutine'},
                    {name: `rakija`, value: 'rakija'},
                )
            )
        )
    ...
    async execute(interaction) {
        ...
        if (subcommand === `create`) {
            ...
            const newGroup = new groupModel({
                name: groupName, owner: interaction.user.id, members: [interaction.user.id], balance: 0
            });

            try {
                // Attempt to create channel in category with ID 1086653364168839268
                const guild = interaction.guild;
                const category = guild.channels.cache.get(`1117517030917148773`);
                const channel = await guild.channels.create({
                    name: groupName, type: ChannelType.GuildText, parent: category, permissionOverwrites: [{ // Owner
                ...
                newGroup.guildID = interaction.guild.id
                await newGroup.save();
        ...
        if (subcommand === 'info') {
            ...
            const embed = new EmbedBuilder()
                .setColor(`#00ff00`)
                .setTitle(`Group Info`)
                .addFields({name: `Name`, value: groupData.name, inline: true}, {
                    name: `Owner`,
                    value: owner.username,
                    inline: true
                }, {name: `Members`, value: members.join(`\n`), inline: true}, {
                    name: `Balance`,
                    value: `$${groupData.balance}`,
                    inline: true
                },)
                .setTimestamp()
                .setFooter({text: `Group ID: ${groupData._id}`});
        ...
        if (subcommand === 'list') {
            ...
            // list all groups
            const groups = await groupModel.find({guildID: interaction.guild.id});
            const groupNames = [];

            for (const group of groups) groupNames.push(`<#${group.channelID}> - ` + `${group.members.length} ${group.members.length === 1 ? "member" : "members"} - ` + `\$${group.balance.toLocaleString()}`)
